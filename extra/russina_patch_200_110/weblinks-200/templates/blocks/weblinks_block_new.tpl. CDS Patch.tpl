<{* $Id: weblinks_block_new.html.\040CDS\040Patch.html,v 1.1 2012/04/09 10:22:24 ohwada Exp $ *}>

<{* google map *}>
<{if $block.show_gmap}>
    <{$block.gmap}>
    <{/if}>

<{if $block.show_recommend}>
<table>
    <tr>
        <td width="50%">
            <{/if}>

            <{* latest links *}>
            <ul>
                <{foreach item=link from=$block.links}>
                <li>
                    <{if $link.show_banner == 1}>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/singlelink.php?lid=<{$link.id}>">
                        <{if $link.width > 0 }>
                    <img src="<{$link.banner}>" width="<{$link.width}>" border="0" alt="banner"/>
                        <{else}>
                    <img src="<{$link.banner}>" border="0" alt="banner"/>
                        <{/if}>
                    </a>
                    <{/if}>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/singlelink.php?lid=<{$link.id}>"><{$link.title}></a> (<{$link.date}>)
                    <{if $link.show_new }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/new.gif" border="0" alt="new"/>
                    <{/if}>
                    <{if $link.show_update }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/update.gif" border="0" alt="update"/>
                    <{/if}>
                    <{if $link.show_pop }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/pop.gif" border="0" alt="popular"/>
                    <{/if}>
                    <{if $link.show_cat_title }>
                <br> <{$smarty.const._WEBLINKS_IN}>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/viewcat.php?cid=<{$link.cid}>"><{$link.cat_title}></a>
                    <{/if}>
                    <{if $link.name != ''}>
                    <{$link.name}>
                    <{/if}>
                    <{if $link.show_desc }>
                <br>
                    <span style="font-size: 90%;">
    <{$link.desc_short}>
    </span>
                    <{/if}>
                </li>
                <{/foreach}>
            </ul>
            <{* latest links end *}>

            <{* recommend links *}>
            <{if $block.show_recommend}>

        </td>
        <td width="50%">
            <ul>
                <{foreach item=link from=$block.recommend_links}>
                <li>
                    <{if $link.show_banner }>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/singlelink.php?lid=<{$link.id}>">
                        <{if $link.width > 0 }>
                    <img src="<{$link.banner}>" width="<{$link.width}>" border="0" alt="banner"/>
                        <{else}>
                    <img src="<{$link.banner}>" border="0" alt="banner"/>
                        <{/if}>
                    </a>
                    <{/if}>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/singlelink.php?lid=<{$link.id}>"><{$link.title}></a> (<{$link.date}>)
                    <{if $link.show_new }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/new.gif" border="0" alt="new"/>
                    <{/if}>
                    <{if $link.show_update }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/update.gif" border="0" alt="update"/>
                    <{/if}>
                    <{if $link.show_pop }>
                <img src="<{$xoops_url}>/modules/<{$block.dirname}>/images/pop.gif" border="0" alt="popular"/>
                    <{/if}>
                    <{if $link.show_cat_title }>
                <br> <{$smarty.const._WEBLINKS_IN}>
                    <a href="<{$xoops_url}>/modules/<{$block.dirname}>/viewcat.php?cid=<{$link.cid}>"><{$link.cat_title}></a>
                    <{/if}>
                    <{if $link.show_desc}>
                <br>
                    <span style="font-size: 90%;">
    <{$link.desc_short}>
    </span>
                    <{/if}>
                </li>
                <{/foreach}>
            </ul>
        </td>
    </tr>
</table>

    <{/if}>
<{* recommend links end *}>
