...
class TableManage extends Happylinux\TableManage
{
    ...
    public function __construct()
    {
        ...
        $this->set_config_define(Weblinks\Config2Define::getInstance(WEBLINKS_DIRNAME));
        $this->set_install_class(Install::getInstance(WEBLINKS_DIRNAME));
        $this->set_xoops_block_checker();

        $this->_zombie = TableManageZombie::getInstance();
    }
    ...
    public function _print_menu_rssc()
    {
        $title = 'RSSC Check';

        echo '<h4>' . $title . "</h4>\n";

        echo $this->_form->build_lib_box_button_style($title, 'check adjustment of Weblinks link table and RSSC link table', 'check_link', _HAPPYLINUX_EXECUTE, 'table_manage_rssc.php');
    }
    ...
    public function _print_menu_xml()
    {
        ...
        echo $this->_form->build_lib_box_button_style($title, 'since v1.20, rss_xml field in link table became unnecessary', 'clean_xml', _HAPPYLINUX_EXECUTE);
    }
    ...
    public function _print_menu_rebuild_search()
    {
        ...
        echo $this->_form->build_lib_box_button_style($title, 'Bug fix : necessary in the version up from v1.82 or before', 'rebuild', _HAPPYLINUX_EXECUTE, 'table_manage_search.php');
    }

    //---------------------------------------------------------
    // action
    //---------------------------------------------------------
    public function clean_xml()
    {
        if (!$this->check_token()) {
            xoops_cp_header();
        ...
        if ($ret) {
            $time = 1;
            $msg = _HAPPYLINUX_EXECUTED;
        } else {
            $time = 3;
            $msg = _HAPPYLINUX_FAILED;
...
$manage = TableManage::getInstance();
