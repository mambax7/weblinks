...
if (!class_exists('weblinksD3commentContent')) {
    ...
    class weblinksD3commentContent extends D3commentAbstract
    {
        ...
        public function validate_id($link_id)
        {
            ...
            list($count) =
                $db->fetchRow($db->query('SELECT COUNT(*) FROM ' . $db->prefix($mydirname . '_link') . " WHERE lid=$weblinks_id AND comment_use AND ( time_publish = 0 OR time_publish < " . $time
                                         . ' ) AND ( time_expire = 0 OR time_expire > ' . $time . ' )'));
        ...
        public function onUpdate($mode, $link_id, $forum_id, $topic_id, $post_id = 0)
        {
            ...
            list($count) = $db->fetchRow($db->query('SELECT COUNT(*) FROM ' . $db->prefix($this->d3forum_dirname . '_posts') . ' p LEFT JOIN ' . $db->prefix($this->d3forum_dirname . '_topics')
                                                    . " t ON t.topic_id=p.topic_id WHERE t.forum_id=$forum_id AND t.topic_external_link_id='$weblinks_id'"));
