...
');
    ...
    define('_WEBLINKS_CAT_IMG_MODE_DSC',
           '「なし」のときは、画像を表示しない。<br />「folder.gif」のときは、folder.gif を表示する。<br />「設定した画像」のときは、カテゴリ毎に設定された画像を表示する。');
    ...
    define('_WEBLINKS_ADMIN_LINK_BROKEN_CHECK_CAUTION',
           'リンク数が多いと、タイムアウトすることがあります。<br />そのときは、limit と offset の数値を変更してください。<br />limit=0 は制限なしです。<br />');
    ...
    define('_AM_WEBLINKS_LINK_IMAGE_AUTO_DSC',
           '「はい」を選択すると、<br />バナー・サイズが登録・変更時に取得できなかった場合に、リンク一覧やリンク詳細を表示するときに、再度 自動的に取得する。<br />');
    ...
    define('_AM_WEBLINKS_CONF_PERFORMANCE_DSC',
           '性能向上のために、表示するときに必要な情報を事前に計算し、データベースに格納します<br />初めて使用するときは、「カテゴリ一覧」->「パス・ツリー情報の更新」を実行すること');
    define('_AM_WEBLINKS_CAT_PATH', 'カテゴリのパス・ツリー情報');
    define('_AM_WEBLINKS_CAT_PATH_DSC',
           '「はい」を選択すると、<br />カテゴリのパス・ツリー情報を事前に計算し、カテゴリ・テーブルに格納します。<br />「いいえ」を選択すると、<br />表示するときに計算します。');
    define('_AM_WEBLINKS_CAT_COUNT', 'カテゴリのリンク数');
    define('_AM_WEBLINKS_CAT_COUNT_DSC',
           '「はい」を選択すると、<br />カテゴリ毎のリンク数を事前に計算し、カテゴリ・テーブルに格納します。<br />「いいえ」を選択すると、<br />表示するときに計算します。');
    ...
    define('_AM_WEBLINKS_GM_USE_GEOCODE_TOKYO_DESC',
           '日本でのみ有効<br />住所から緯度・経度を検索する<br /><a href="http://pc035.tkl.iis.u-tokyo.ac.jp/~sagara/geocode/" target="_blank">東京大学 シンプルジオコーディング実験</a>');
    define('_AM_WEBLINKS_GM_USE_NISHIOKA', '[検索][日本] 逆ジオコードを使用する');
    define('_AM_WEBLINKS_GM_USE_NISHIOKA_DESC',
           '日本でのみ有効<br />緯度・経度から住所を検索する<br /><a href="http://nishioka.sakura.ne.jp/google/" target="_blank">http://nishioka.sakura.ne.jp/google/</a>');
    ...
    define('_AM_WEBLINKS_HEADER_MODE_DESC',
           '「いいえ」のときは、スタイルシートと Javascript の指定を body タグ内に表示します<br />「はい」のときは、xoops module header を使用して、header タグ内に表示します<br />テーマによっては使用できないものがあります');
    ...
    define('_AM_WEBLINKS_USE_HITS_SINGLELINK_DSC',
           '「ジャンプの "ヒット数" の使用」が「はい」のときに有効<br />「はい」を選択すると、<br />リンク詳細 singlelink を表示したときに、ヒット数 がカウントアップされます');
...
WEBサイトのサムネイルを表示する');
    ...
    define('_AM_WEBLINKS_BULK_COMMENT_DSC1',
           'リンクのタイトル, uid, コメントのタイトル, コメントの本文 をカンマ(,)で区切って記述する<br />uid は省略可。管理者のuid で代用される<br/ >コメントのタイトル は省略可。リンクのタイトル で代用される');
