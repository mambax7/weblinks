<?php
// $Id: mailusers.php,v 1.1 2012/04/09 10:20:06 ohwada Exp $

//=========================================================
// WebLinks Module
// 2007-07-16 K.OHWADA
//=========================================================

//---------------------------------------------------------
// this file is for XC 2.1
// dont use this in XOOPS 2.0.x
// same as modules/system/language/english/admin/mailusers.php
//---------------------------------------------------------
// _LANGCODE: ru
// _CHARSET : cp1251
// Translator: Houston (Contour Design Studio https://www.cdesign.ru/)

//%%%%%%    Admin Module Name  MailUsers    %%%%%
define('_AM_DBUPDATED', _MD_AM_DBUPDATED);

//%%%%%%    mailusers.php   %%%%%
define('_AM_SENDTOUSERS', '��������� ��������� �������������, �������:');
define('_AM_SENDTOUSERS2', '��������� ���:');
define('_AM_GROUPIS', '������ � ������ (�����������)');
define('_AM_TIMEFORMAT', '(������ ����-��-��, �����������)');
define('_AM_LASTLOGMIN', '��������� ��� ������� �����');
define('_AM_LASTLOGMAX', '��������� ��� ������� ��');
define('_AM_REGDMIN', '������������������ �����');
define('_AM_REGDMAX', '������������������ ��');
define('_AM_IDLEMORE', '��������� ��� ������� ����� ��� X ���� ����� (�����������)');
define('_AM_IDLELESS', '��������� ��� ������� ����� ��� X ���� ����� (�����������)');
define('_AM_MAILOK', '��������� ��������� ������ ������������� ����������� �������������� (�����������)');
define('_AM_INACTIVE', '��������� ��������� ������ ���������� ������������� (�����������)');
define('_AMIFCHECKD', '���� ��� ��������, �� ��� ������������� � ������ ��������� ����� ��������������');
define('_AM_MAILFNAME', '�� ����� (������ ����������� �����)');
define('_AM_MAILFMAIL', '�� ������ ����������� ����� (������ ����������� �����)');
define('_AM_MAILSUBJECT', '����');
define('_AM_MAILBODY', '���������');
define('_AM_MAILTAGS', '������������ ����:');
define('_AM_MAILTAGS1', '{X_UID} ����� ��������� ��� ID ������������');
define('_AM_MAILTAGS2', '{X_UNAME} ����� ��������� ��� ��� ������������');
define('_AM_MAILTAGS3', '{X_UEMAIL} ����� ��������� ��� ����������� ����� ������������');
define('_AM_MAILTAGS4', '{X_UACTLINK} ����� ��������� ��� ������ �� ��������� ������������');
define('_AM_SENDTO', '��������� ���');
define('_AM_EMAIL', '����������� �����');
define('_AM_PM', '������ ���������');
define('_AM_SENDMTOUSERS', '�������� ��������� �������������');
define('_AM_SENT', '���������� �������������');
define('_AM_SENTNUM', '%s - %s (�����: %s �������������)');
define('_AM_SENDNEXT', '������');
define('_AM_NOUSERMATCH', '��� ���������� �������������');
define('_AM_SENDCOMP', '�������� ��������� ���������.');
